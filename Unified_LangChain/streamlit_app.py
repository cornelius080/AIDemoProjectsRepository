...
MODELS = {
    "Ollama": ["nemotron-3-super:cloud", "minimax-m2.5:cloud", "gemma4:31b-cloud", "qwen3-coder-next:cloud",],
    "OpenAI": ["gpt-5.5", "gpt-5.4-nano", "gpt-5-nano", "gpt-4.1"],
    "Google": ["gemini-3.1-pro-preview", "gemini-3.1-flash-lite", "gemini-2.5-pro", "gemini-2.5-flash"],
    "Anthropic": ["claude-opus-4-7", "claude-sonnet-4-6", "claude-haiku-4-5", "claude-sonnet-3-7"],
}
...
def get_agent(model_provider: str, model_name: str, temperature: float):
    ...
    return RagWebAgent(model_provider=model_provider, model_name=model_name, temperature=temperature)
...
def extract_from_webpages():
    ...
    if selected_urls:
        with st.spinner("Extracting content from selected pages..."):
            ...
            if not extracted_data:
                st.warning("No content could be extracted from these sources. Try with different URLs or advanced search depth.", icon="⚠️")
                return
    ...
    st.session_state.is_chat_visibile = True
    st.session_state.messages = []
...
def search_on_click():
    ...
    settings = {
        ...
        "include_image_descriptions": st.session_state.get("include_image_descriptions"),
        "include_domains": [d.strip() for d in st.session_state.get("include_domains", "").split(",") if d.strip()] if st.session_state.get("include_domains") else [],
        "exclude_domains": [d.strip() for d in st.session_state.get("exclude_domains", "").split(",") if d.strip()] if st.session_state.get("exclude_domains") else [],
    ...
    # Print all parameters to console as requested
    print("\n--- Search Triggered ---")
    ...
    st.session_state.is_chat_visibile = False
    st.session_state.messages = []
...
# HEADER
if "is_chat_visibile" not in st.session_state:
    st.session_state.is_chat_visibile = False
...
# FOLLOW-UP CHAT
if st.session_state.is_chat_visibile:
